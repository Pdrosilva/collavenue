            ...
            reader.onload = (ev) => {
                ...
                img.onload = async () => {
                    ...
                    if (viewTarget === "detail" && selectedImage && canvasRef.current) {
                        ...
                        // Because the UI treats sub-images as workspaces with x,y, 
                        // we must wait for Supabase to support x,y in the table if it's truly nested. 
                        // But for now, we just insert as a workspace and store x, y in local state 
                        // (our schema didn't include x,y on workspaces, so we'll mock it locally for now)
                    }
